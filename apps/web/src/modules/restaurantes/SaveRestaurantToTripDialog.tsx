...
interface SaveRestaurantToTripDialogProps {
  readonly visit: RestaurantVisit;
  readonly cost: RestaurantCost;
}

export function SaveRestaurantToTripDialog({ visit, cost }: SaveRestaurantToTripDialogProps) {
  ...
  const [newTripTitle, setNewTripTitle] = useState(visit.placeName);
  const [newDayDate, setNewDayDate] = useState(visit.date ?? '');
  ...
  async function onSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const trip = await (selectedTripId === NEW_TRIP
        ? createTrip(auth.getToken, { title: newTripTitle.trim() || visit.placeName })
        : getTrip(auth.getToken, selectedTripId));
      ...
      setError(cause instanceof Error ? cause.message : 'Não foi possível salvar o restaurante.');
      setStatus('error');
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Salvar restaurante na viagem</DialogTitle>
          <DialogDescription>{visit.placeName}</DialogDescription>
        </DialogHeader>

        <form className="grid gap-4" onSubmit={onSave}>
          <div className="grid gap-1.5">
            <Label htmlFor="save-restaurant-trip">Viagem</Label>
            <Select
              id="save-restaurant-trip"
          ...
          {selectedTripId === NEW_TRIP ? (
            <div className="grid gap-1.5">
              <Label htmlFor="save-restaurant-trip-title">Título da viagem</Label>
              <Input
                id="save-restaurant-trip-title"
                value={newTripTitle}
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="save-restaurant-day">Dia</Label>
            <Select
              id="save-restaurant-day"
              value={selectedDayId}
          ...
          {selectedDayId === NEW_DAY ? (
            <div className="grid gap-1.5">
              <Label htmlFor="save-restaurant-day-date">Data do dia</Label>
              <Input
                id="save-restaurant-day-date"
                type="date"
                value={newDayDate}
          ...
          {status === 'saved' && savedTripId ? (
            <p className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
              Restaurante salvo.{' '}
              <Link className="font-semibold underline" to={`${TRIPS_PATH}/${savedTripId}`}>
                Abrir viagem
              </Link>
            </p>
          ) : null}
