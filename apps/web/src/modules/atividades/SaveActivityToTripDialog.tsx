...
interface SaveActivityToTripDialogProps {
  readonly plan: ActivityPlan;
  readonly totalCost: number;
}

export function SaveActivityToTripDialog({ plan, totalCost }: SaveActivityToTripDialogProps) {
  ...
  const [newTripTitle, setNewTripTitle] = useState(plan.placeName);
  const [newDayDate, setNewDayDate] = useState(plan.date ?? '');
  ...
  async function onSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const trip = await (selectedTripId === NEW_TRIP
        ? createTrip(auth.getToken, { title: newTripTitle.trim() || plan.placeName })
        : getTrip(auth.getToken, selectedTripId));
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Salvar atividade na viagem</DialogTitle>
          <DialogDescription>{plan.placeName}</DialogDescription>
        </DialogHeader>

        <form className="grid gap-4" onSubmit={onSave}>
          <div className="grid gap-1.5">
            <Label htmlFor="save-activity-trip">Viagem</Label>
            <Select
              id="save-activity-trip"
          ...
          {selectedTripId === NEW_TRIP ? (
            <div className="grid gap-1.5">
              <Label htmlFor="save-activity-trip-title">Título da viagem</Label>
              <Input
                id="save-activity-trip-title"
                value={newTripTitle}
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="save-activity-day">Dia</Label>
            <Select
              id="save-activity-day"
              value={selectedDayId}
          ...
          {selectedDayId === NEW_DAY ? (
            <div className="grid gap-1.5">
              <Label htmlFor="save-activity-day-date">Data do dia</Label>
              <Input
                id="save-activity-day-date"
                type="date"
                value={newDayDate}
