...
interface SaveStayToTripDialogProps {
  readonly stay: LodgingStay;
  readonly totalCost: number;
}

export function SaveStayToTripDialog({ stay, totalCost }: SaveStayToTripDialogProps) {
  ...
  const [newTripTitle, setNewTripTitle] = useState(stay.placeName);
  const [newDayDate, setNewDayDate] = useState(stay.checkIn);
  ...
  async function onSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const trip = await (selectedTripId === NEW_TRIP
        ? createTrip(auth.getToken, { title: newTripTitle.trim() || stay.placeName })
        : getTrip(auth.getToken, selectedTripId));
      ...
      setError(cause instanceof Error ? cause.message : 'Não foi possível salvar a hospedagem.');
      setStatus('error');
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Salvar hospedagem na viagem</DialogTitle>
          <DialogDescription>{stay.placeName}</DialogDescription>
        </DialogHeader>

        <form className="grid gap-4" onSubmit={onSave}>
          <div className="grid gap-1.5">
            <Label htmlFor="save-stay-trip">Viagem</Label>
            <Select
              id="save-stay-trip"
          ...
          {selectedTripId === NEW_TRIP ? (
            <div className="grid gap-1.5">
              <Label htmlFor="save-stay-trip-title">Título da viagem</Label>
              <Input
                id="save-stay-trip-title"
                value={newTripTitle}
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="save-stay-day">Dia</Label>
            <Select
              id="save-stay-day"
              value={selectedDayId}
          ...
          {selectedDayId === NEW_DAY ? (
            <div className="grid gap-1.5">
              <Label htmlFor="save-stay-day-date">Data do dia</Label>
              <Input
                id="save-stay-day-date"
                type="date"
                value={newDayDate}
